...
manipulated_data3 <- joined_data %>%
  ...
  pivot_longer(cols=c(11:19), values_to="Count") %>%
  ...
  summarize(Janssen=sum(Janssen), Moderna=sum(Moderna), Pfizer=sum(Pfizer), Unknown=sum(UnkManuf)) %>%
  pivot_longer(cols=c(Janssen, Moderna, Pfizer, Unknown), names_to="Brand", values_to="Count") %>%
  unite(Index, c(Year, Month), sep=".", remove = F)
